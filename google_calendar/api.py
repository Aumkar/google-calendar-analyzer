...
class ReportCalculator(object):
    ...
    def time_spent(self):
        ...
        sorted_by_count_df['count'] = sorted_by_count_df['duration'].astype(
            np.timedelta64
        )
        sorted_by_count_df['month'] = sorted_by_count_df.apply(
            lambda x: f"{x['year']}-{x['month']}", axis=1
        ...
        result['least_time_spent'] = most_time_spent.to_dict(orient='r')
...
def calculate_stats(user, filter_keyword=None):
    """
    Calculates stats for below metrics
    - Number of meetings
    - Total time spent for meetings
    - Top attendees
    ...
    if filter_keyword:
        event_filter['summary__icontains'] = filter_keyword
        attendee_filter['event__summary__icontains'] = filter_keyword
    ...
    # Creating monthly events df
    monthly_events_df = pd.DataFrame(list(
    ...
    monthly_events_df.rename(columns={
        'start_datetime__year': 'year',
        'start_datetime__month': 'month',
    }, inplace=True)

    # Creating weekly events df
    weekly_events_df = pd.DataFrame(list(
        event_queryset.values(
            'start_datetime__year',
            'start_datetime__week'
    ...
    weekly_events_df.rename(columns={
        'start_datetime__year': 'year',
        'start_datetime__week': 'week',
    }, inplace=True)

    last_3_month_date = None
    result = {}
    # Number of meetings
    month_events_sorted_by_number_of_meetings = monthly_events_df.sort_values(
        'count'
    )
    least_meetings = month_events_sorted_by_number_of_meetings.iloc[1]
    most_meetings = month_events_sorted_by_number_of_meetings.iloc[-1]
    last_3_months_df = monthly_events_df[
        (monthly_events_df['year'] >= last_3_month_date.year) &
        (monthly_events_df['month'] >= last_3_month_date.month)
    ][['month', 'year', 'count']]
    if not last_3_months_df.empty:
        ...
        del last_3_months_df['year']
    result['meetings'] = {
        'total': monthly_events_df['count'].sum(),
        'last_3_months': last_3_months_df.to_dict(orient='r'),
        'most_meetings':{
            'month': f"{most_meetings['year']}-{most_meetings['month']}",
            'count': most_meetings['count']
        } ,
        'least_meetings':{
            'month': f"{least_meetings['year']}-{least_meetings['month']}",
            'count': least_meetings['count']
        },
        'weekly_average': weekly_events_df['count'].mean().round(2)
    }
    ...
    result['time_spent'] = {
        'total': str(monthly_events_df['duration'].sum()),
        'last_3_months': last_3_months_df.to_dict(orient='r'),
        'most_time_spent': {
            'month': f"{most_meetings['year']}-{most_meetings['month']}",
            'duration': str(most_meetings['duration'])
        },
        'least_time_spent': {
            'month': f"{least_meetings['year']}-{least_meetings['month']}",
            'duration': str(least_meetings['duration'])
        },
        'weekly_average': weekly_average
    }

    # Attendee metrics
    attendee_count_series = pd.Series(dict(
    ...
    result['top_people'] = [
        {'name': key, 'count': val} for key, val
        in attendee_count_series.nlargest(3, keep='all').items()
    ]
    return result
